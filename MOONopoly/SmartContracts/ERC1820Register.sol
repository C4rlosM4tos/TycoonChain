...
// import "browser/interfaceOwner.sol";
    ...
    event newContractSet (string name, address contractSet);
       ...
       constructor (address _PolyRegister) public {
        ...
        registerPoly = PolyRegister(_PolyRegister);
              ...
              (bool[] memory boolSolutions, address[] memory solutions) = registerPoly.registerContract("Registry", needs);
          if(solutions.length > 0) {
           ...
           (address[] memory contractsInNeed, uint[] memory hashesOfContractTypesinNeed) = registerPoly.fillOthersNeedsIfPossible(uint(keccak256(abi.encodePacked("Registry"))), address(this));
...
function checkHash (uint hash, address foundAt ) public returns (bool) {
   ...
   // uint hashBoard = uint(keccak256(abi.encodePacked("Boards")));
    if (hash == hashAuction) {
  //   bool answer = Boards(foundAt).setStreetsContract(address(this));
